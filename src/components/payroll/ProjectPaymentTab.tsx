        ...
        {historyLoading ? (
          ...
          <View>
            {filteredHistory.map((record, index) => {
              const id = record.uuid || record.id;
              return (
                <View key={id || index} className="flex-row items-center justify-between p-4 mb-3 bg-white rounded-2xl shadow-sm" style={{ shadowColor: "#f97316", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.08, shadowRadius: 8, elevation: 3 }}>
                  ...
                  <View className="items-end gap-2">
                    <Text className="font-black text-orange-600 text-base">₹{parseFloat(record.amount_paid).toFixed(2)}</Text>
                    <View className="flex-row gap-1">
                      ...
                      <TouchableOpacity onPress={() => handleDelete(id)} className="bg-orange-50 p-1.5 rounded-md">
