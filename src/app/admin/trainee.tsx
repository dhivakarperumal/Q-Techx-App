...
import { ActivityIndicator, Alert, Modal, Pressable, RefreshControl, ScrollView, Text, TextInput, View, TouchableOpacity } from "react-native";
...
function TaskMasterModal({ visible, onClose, onSaved }: { visible: boolean; onClose: () => void; onSaved: () => void }) {
  ...
  return <Modal visible={visible} animationType="slide" onRequestClose={onClose}><View className="flex-1 bg-[#f8fafc]"><View className="flex-row items-center justify-between border-b border-slate-200 bg-white px-5 pb-4 pt-5"><View><Text className="text-xl font-black text-slate-900">Create Task</Text><Text className="mt-1 text-xs text-slate-500">Add a reusable task for trainees and interns.</Text></View><Pressable onPress={onClose}><Ionicons name="close-circle" size={28} color="#94a3b8" /></Pressable></View><ScrollView contentContainerStyle={{ padding: 20 }}><View className="rounded-3xl bg-white p-4 shadow-sm"><Field label="Task Name *" value={taskName} onChange={setTaskName} placeholder="Enter task name" /><Field label="Description" value={description} onChange={setDescription} placeholder="Optional task description" multiline /><Pressable disabled={saving} onPress={save} className="items-center rounded-xl bg-orange-500 py-3 disabled:opacity-50"><Text className="font-bold text-white">{saving ? "Saving..." : "Save Task"}</Text></Pressable></View></ScrollView></View></Modal>;
}
        ...
        <View className="mb-6">
          ...
          <View className="flex-row gap-3">
            <View className="flex-1">
              <Pressable onPress={() => { /* open type picker */ }} className="h-11 bg-white border border-slate-200 rounded-xl px-3 flex-row items-center justify-between">
                <Text className="text-xs font-medium text-slate-700">{type || 'All'}</Text>
                <Ionicons name="chevron-down" size={15} color="#64748b" />
              </Pressable>
            </View>
            <View className="flex-1">
              <Pressable onPress={() => { /* open status picker */ }} className="h-11 bg-white border border-slate-200 rounded-xl px-3 flex-row items-center justify-between">
