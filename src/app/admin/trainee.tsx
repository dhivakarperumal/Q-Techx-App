...
function AssignmentModal({ visible, member, employees, onClose, onSaved }: { visible: boolean; member: Member | null; employees: Employee[]; onClose: () => void; onSaved: () => void }) {
  ...
  return <Modal visible={visible} animationType="slide" transparent onRequestClose={onClose}><View className="flex-1 justify-end bg-black/50"><View className="bg-white rounded-t-3xl h-[85%] shadow-2xl overflow-hidden"><View className="bg-black pt-4 px-6 pb-6"><View className="w-12 h-1.5 bg-slate-700 rounded-full mx-auto mb-4" /><View className="flex-row justify-between items-center"><View><Text className="text-lg font-black text-orange-500">Assign Employee</Text><Text className="mt-1 text-xs text-white/70">{member ? nameOf(member) : ""}</Text></View><Pressable onPress={onClose}><Ionicons name="close-circle" size={28} color="#f97316" /></Pressable></View></View><View className="flex-1"><ScrollView contentContainerStyle={{ padding: 20, paddingTop: 0 }}>{employees.map((employee) => <Pressable key={String(employee.employee_id)} onPress={() => setEmployeeId(String(employee.employee_id))} className={`mb-2 flex-row items-center rounded-2xl border p-3 ${employeeId === String(employee.employee_id) ? "border-orange-400 bg-orange-50" : "border-slate-200 bg-white"}`}><View className="h-10 w-10 items-center justify-center rounded-xl bg-blue-50"><Text className="font-black text-blue-600">{initials(employeeName(employee))}</Text></View><View className="ml-3 flex-1"><Text className="font-bold text-slate-800">{employeeName(employee)}</Text><Text className="mt-1 text-xs text-slate-500">{employee.employee_code || ""}{employee.designation ? ` - ${employee.designation}` : ""}</Text></View><Ionicons name={employeeId === String(employee.employee_id) ? "radio-button-on" : "radio-button-off"} size={21} color={employeeId === String(employee.employee_id) ? "#f97316" : "#94a3b8"} /></Pressable>)}</ScrollView></View><Pressable disabled={saving} onPress={submit} className="mx-5 mb-5 items-center rounded-xl bg-orange-500 py-3"><Text className="font-bold text-white">{saving ? "Saving..." : "Assign / Reassign"}</Text></Pressable></View></View></Modal>;
}
