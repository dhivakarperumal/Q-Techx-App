...
const stats = [
  ...
  { label: 'In Progress', value: '42', sub: '48.8%', icon: 'briefcase', color: '#3b82f6', bg: 'bg-blue-50' },
  { label: 'Completed', value: '28', sub: '32.6%', icon: 'checkmark-circle', color: '#10b981', bg: 'bg-green-50' },
  { label: 'On Hold', value: '16', sub: '18.6%', icon: 'pause-circle', color: '#a855f7', bg: 'bg-purple-50' },
];
  ...
  return (
    <View className="flex-1 bg-[#F9FAFB]">
      ...
      <ScrollView className="flex-1" contentContainerClassName="pb-32 pt-2">
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerClassName="px-5 mb-6" className="overflow-visible">
          {stats.map((stat, idx) => (
            <View key={idx} className="bg-white rounded-[24px] p-4 mr-4 border border-slate-100 shadow-sm w-[130px]">
              ...
              <Text className="text-slate-400 text-[10px] font-medium">{stat.sub}</Text>
            </View>
          ))}
        </ScrollView>
        ...
        {/* ── FILTER PILLS ── */}
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerClassName="px-5 mb-6">
        ...
        <View className="px-5">
          {projects.map((project, idx) => (
            <View key={idx} className="bg-white rounded-3xl p-4 mb-4 border border-slate-100 shadow-sm">
              <View className="flex-row items-start justify-between mb-4">
                <View className="flex-row items-center flex-1">
                  <View className={`w-14 h-14 rounded-[18px] ${project.iconBg} items-center justify-center mr-4`}>
                    <Ionicons name={project.icon as any} size={28} color={project.iconColor} />
                  </View>
                  <View className="flex-1">
                    <Text className="text-slate-900 font-bold text-base mb-1">{project.title}</Text>
                    <View className="flex-row items-center mb-1">
                      <Ionicons name="business-outline" size={12} color="#94a3b8" />
                      <Text className="text-slate-500 text-xs ml-1">{project.company}</Text>
                    </View>
                    <View className="flex-row items-center">
                      <Ionicons name="calendar-outline" size={12} color="#94a3b8" />
                      <Text className="text-slate-500 text-xs ml-1">{project.date}</Text>
                    </View>
                  </View>
                </View>
                ...
                <View className="items-end justify-between h-14 py-1">
                  ...
                  <View className={`px-2 py-1 rounded-md ${project.statusBg}`}>
                    <Text className={`text-[10px] font-bold ${project.statusColor}`}>{project.status}</Text>
                  </View>
                </View>
              </View>
              ...
              <View className="flex-row items-center mt-2">
                ...
                <Text className="text-slate-600 font-bold text-xs" style={{ color: project.progressColor }}>
                  {project.progress}%
                </Text>
              </View>
            </View>
