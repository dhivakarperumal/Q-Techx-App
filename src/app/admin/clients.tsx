...
export default function ClientsScreen() {
  ...
  const displayedClients = clients.filter(client => {
    ...
    return client.follow_up_status !== "Completed" && client.follow_up_status !== "Cancelled";
  });
  
  const activeCount = clients.filter((client) => client.client_status === "Active").length;

  const pendingCount = clients.filter(c => c.follow_up_status !== "Completed" && c.follow_up_status !== "Cancelled").length;

  return <View className="flex-1 bg-[#f8fafc]"><TopHeader /><ScrollView className="flex-1" contentContainerStyle={{ paddingHorizontal: 20, paddingTop: 14, paddingBottom: 120 }} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => loadClients(true)} tintColor="#f97316" />}><View className="flex-row items-end justify-between"><View><Text className="text-3xl font-black text-slate-900">Clients</Text><Text className="mt-1 text-sm text-slate-500">Manage client relationships, documents, and follow-ups.</Text></View></View><View className="mt-5 flex-row gap-3"><View className="flex-1 rounded-2xl bg-white p-4 shadow-sm"><Text className="text-xs font-bold text-slate-400">TOTAL CLIENTS</Text><Text className="mt-1 text-2xl font-black text-slate-900">{clients.length}</Text></View><View className="flex-1 rounded-2xl bg-white p-4 shadow-sm"><Text className="text-xs font-bold text-slate-400">ACTIVE</Text><Text className="mt-1 text-2xl font-black text-emerald-600">{activeCount}</Text></View></View>
  ...
  <View className="mt-6 flex-row rounded-xl bg-slate-200 p-1">
    <Pressable onPress={() => setActiveTab("All Clients")} className={`flex-1 items-center rounded-lg py-2.5 ${activeTab === "All Clients" ? "bg-white shadow-sm" : "bg-transparent shadow-none"}`}>
      <Text className={`text-sm font-bold ${activeTab === "All Clients" ? "text-slate-900" : "text-slate-500"}`}>All Clients ({clients.length})</Text>
    </Pressable>
