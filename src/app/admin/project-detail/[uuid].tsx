    ...
    try {
      ...
      if (assignRes.status === "fulfilled") {
        ...
        const extract = (res: any) => {
          ...
          return payload?.employees || payload?.assignments || payload?.rows || payload?.data || [];
        };
        let rows = extract(assignRes.value.data);
        if (!Array.isArray(rows)) {
          if (rows?.data && Array.isArray(rows.data)) rows = rows.data;
          else if (rows?.employees && Array.isArray(rows.employees)) rows = rows.employees;
          else if (rows?.assignments && Array.isArray(rows.assignments)) rows = rows.assignments;
          else rows = [];
        }
        setAssignments(rows);
      }
      if (taskRes.status === "fulfilled") {
        ...
        let rows = extract(taskRes.value.data);
        if (!Array.isArray(rows)) {
          if (rows?.data && Array.isArray(rows.data)) rows = rows.data;
        ...
        let filteredRows = rows.filter((t: any) => {
          ...
          if (!possibleName && typeof t.project === "string" && isNaN(Number(t.project))) {
            possibleName = t.project; // If it's a non-numeric string, it might be the project name
          }
      ...
      >
        <SafeAreaView edges={["top"]}>
          ...
          >
            ...
            >
              <Text style={{ fontSize: 11, fontWeight: "700", color: sColor }}>
                {statusStr}
              </Text>
            </View>
            <View
      ...
      >
        ...
        <View style={{ flexDirection: "row", gap: 10, marginBottom: 14 }}>
          {[
            ...
            >
              <Text style={{ fontSize: 20, fontWeight: "900", color: s.color }}>
                {s.value}
              </Text>
              <Text
                style={{
        ...
        <Card>
          ...
          {tasks.length === 0 ? (
            ...
            tasks.slice(0, 15).map((task: any, i: number) => {
              ...
              const isWip = ["in progress", "in-progress"].includes(
                tStatus.toLowerCase(),
              );
            ...
            >
                ...
                {ROLES.map((r) => (
                  <Pressable
                    ...
                    style={{
                      ...
                      borderWidth: 1.5,
                      backgroundColor:
                        selectedRole === r ? "#fff7ed" : "#f8fafc",
                      borderColor: selectedRole === r ? "#f97316" : "#e2e8f0",
                    }}
                  >
                    <Text
                      style={{
                        fontSize: 12,
                        fontWeight: "700",
                        color: selectedRole === r ? "#f97316" : "#64748b",
                      }}
                    >
                      {r}
                    </Text>
              ...
              <View style={{ flexDirection: "row", flexWrap: "wrap" }}>
                {employees
                  ...
                  .map((e) => {
                    ...
                    const color =
                      palette[(e.name?.charCodeAt(0) ?? 0) % palette.length];
                      ...
                      >
                        <View
                          style={{
                            ...
                            height: 28,
                            borderRadius: 9,
                            backgroundColor: sel ? color : "#e2e8f0",
                            alignItems: "center",
                            justifyContent: "center",
                          }}
                        >
              ...
              <Pressable
                ...
                style={{
                  ...
                  alignItems: "center",
                  backgroundColor:
                    assigning || !selectedEmpIds.length ? "#e2e8f0" : "#f97316",
                  shadowColor: "#f97316",
                  shadowOffset: { width: 0, height: 4 },
                  shadowOpacity: assigning || !selectedEmpIds.length ? 0 : 0.3,
                  shadowRadius: 10,
                  elevation: assigning || !selectedEmpIds.length ? 0 : 5,
                }}
              >
                <Text
                  style={{
                    ...
                    fontWeight: "800",
                    color:
                      assigning || !selectedEmpIds.length ? "#94a3b8" : "#fff",
                  }}
                >
                  {assigning
                    ? "Assigning…"
                    : selectedEmpIds.length
                      ? `Assign ${selectedEmpIds.length} Employee${selectedEmpIds.length > 1 ? "s" : ""}`
                      : "Select Employees"}
                </Text>
              </Pressable>
            </ScrollView>
