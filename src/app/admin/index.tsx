      ...
      >
        ...
        <View className="flex-row items-center justify-between mb-6">
          <View>
            ...
            <View className="flex-row items-center mt-1">
              <Text className="text-slate-900 text-3xl font-black mr-2">
                Admin
              </Text>
        ...
        >
          <View className="flex-1">
            ...
            <Text className="text-white/80 text-xs pr-4 leading-5">
              Monitor your system and manage everything with ease.
            </Text>
          </View>
        ...
        <View className="flex-row flex-wrap justify-between mb-6">
          {dashboardStats.map((stat) => (
            <View
              key={stat.label}
              className="w-[48%] bg-white rounded-3xl p-4 mb-4 shadow-sm border border-slate-100"
            >
              <View
                className={`w-12 h-12 ${stat.background} rounded-2xl items-center justify-center mb-3`}
              >
                <Ionicons
        ...
        <View className="mb-6">
          ...
          <View className="flex-row flex-wrap justify-between">
            {quickActions.map((action, index) => (
              <TouchableOpacity
                key={index}
                className="w-[23%] bg-white rounded-[20px] p-3 items-center justify-center shadow-sm border border-slate-100 mb-3"
                style={{ height: 90 }}
              >
                <Ionicons
                  ...
                  color={action.color}
                />
                <Text className="text-slate-700 text-[10px] font-bold mt-2 text-center">
                  {action.label}
                </Text>
              </TouchableOpacity>
            ))}
        ...
        <View className="mb-6">
          ...
          <View className="bg-white rounded-3xl p-2 shadow-sm border border-slate-100">
            {recentActivity.map((activity, index) => (
              <View
                key={index}
                className={`flex-row items-center py-4 px-3 ${index !== recentActivity.length - 1 ? "border-b border-slate-100" : ""}`}
              >
                <View
                  className={`w-12 h-12 rounded-2xl ${activity.iconBg} items-center justify-center mr-4`}
                >
                  <Ionicons
                ...
                <View className="items-end">
                  <Text className="text-slate-400 text-[10px] font-medium mb-2">
                    {activity.time}
                  </Text>
                  <View
                    className={`w-2 h-2 rounded-full ${activity.statusDot}`}
                  />
                </View>
              </View>
            ))}
