...
export default function AdminDashboard() {
  ...
  return <View className="flex-1 bg-white"><TopHeader /><ScrollView className="flex-1" contentContainerStyle={{ paddingHorizontal: 20, paddingTop: 12, paddingBottom: 120 }} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => fetchDashboard(true)} tintColor="#f97316" />}>
    ...
    <LinearGradient colors={["#fdba74", "#52525b"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} className="mb-5 overflow-hidden rounded-[28px] p-5 shadow-lg shadow-black/20"><View className="flex-row items-start justify-between"><View className="flex-1 pr-4"><Text className="text-xs font-bold uppercase tracking-[2px] text-orange-900">Financial Overview</Text><Text className="mt-2 text-2xl font-black text-black">Revenue Insights</Text><Text className="mt-1 text-sm text-gray-900">Live income and payroll snapshot.</Text></View><Ionicons name="trending-up" size={28} color="#000000" /></View><View className="mt-5 flex-row gap-3"><View className="flex-1 rounded-2xl bg-black/10 p-3"><Text className="text-[10px] font-bold uppercase text-gray-800">Income</Text><Text className="mt-1 text-xl font-black text-black">{money(income)}</Text></View><View className="flex-1 rounded-2xl bg-black/10 p-3"><Text className="text-[10px] font-bold uppercase text-gray-800">Payroll</Text><Text className="mt-1 text-xl font-black text-black">{money(dashboard?.monthlyPayroll)}</Text></View></View><View className="mt-4"><View className="mb-1 flex-row justify-between"><Text className="text-[10px] font-bold text-gray-800">Monthly target: Rs 5.0L</Text><Text className="text-[10px] font-black text-black">{targetPercentage}%</Text></View><View className="h-2 overflow-hidden rounded-full bg-black/10"><View className="h-full rounded-full bg-black" style={{ width: `${targetPercentage}%` }} /></View></View></LinearGradient>
