...
export default function AdminDashboard() {
  ...
  return <View className="flex-1 bg-[#f8fafc]"><TopHeader /><ScrollView className="flex-1" contentContainerStyle={{ paddingHorizontal: 20, paddingTop: 12, paddingBottom: 120 }} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => fetchDashboard(true)} tintColor="#f97316" />}>
    ...
    <View className="mb-4 rounded-3xl border border-slate-100 bg-white p-5 shadow-sm"><SectionHeader title="Upcoming Events" action="Calendar" onPress={() => router.push("/admin/calendar")} />{(dashboard?.upcomingEvents || []).length === 0 ? <Text className="text-sm text-slate-400">No upcoming events.</Text> : dashboard?.upcomingEvents?.map((event, index) => <View key={`${event.title}-${index}`} className="mb-3 flex-row items-center rounded-2xl bg-slate-50 p-3"><View className="mr-3 h-12 w-12 items-center justify-center rounded-xl bg-orange-100"><Ionicons name="calendar-outline" size={20} color="#f97316" /></View><View className="flex-1"><Text className="font-bold text-slate-800" numberOfLines={1}>{event.title || "Event"}</Text><Text className="mt-1 text-xs text-slate-500">{dateLabel(event.startDate)}{event.startTime ? ` - ${event.startTime}` : " - All day"}</Text></View><Ionicons name="chevron-forward" size={16} color="#94a3b8" /></View>)}</View>
