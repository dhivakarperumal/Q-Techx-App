...
export default function LoginScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const message = error?.message || (error instanceof Error ? error.message : "Login failed");
    ...
    >
      ...
      >
        ...
        <View className="rounded-[28px] border border-gray-800 bg-[#181a1f] p-6">
          ...
          <Text className="mb-2 text-sm font-medium text-white">Email Address</Text>
          ...
          {fieldError ? <Text className="mt-2 text-xs text-red-400">{fieldError}</Text> : null}
          {serverError ? <Text className="mt-3 text-center text-sm text-red-400">{serverError}</Text> : null}

          <View className="mt-5 flex-row items-center justify-between">
            <Pressable className="flex-row items-center" onPress={() => setRememberMe((value) => !value)}>
              <View className={rememberMe ? "h-5 w-5 items-center justify-center rounded border border-orange-500 bg-orange-500" : "h-5 w-5 rounded border border-gray-600 bg-[#101215]"}>
                {rememberMe ? <Ionicons name="checkmark" size={14} color="white" /> : null}
              </View>
              <Text className="ml-2 text-sm text-gray-400">Remember me</Text>
            </Pressable>
            <Pressable onPress={() => Alert.alert("Forgot password", "Please contact your administrator.")}>
          ...
          >
            ...
            {!isSubmitting ? <Ionicons name="arrow-forward" size={20} color="white" style={{ marginLeft: 8 }} /> : null}
          </Pressable>
          ...
          >
            <Text className="text-sm text-gray-500">
              Don&apos;t have an account? <Text className="text-orange-500">Contact Administrator</Text>
            </Text>
          </Pressable>

          <Pressable className="mt-5 flex-row items-center justify-center" onPress={() => router.replace("/login")}>
