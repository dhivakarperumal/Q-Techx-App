...
const normalizeEmployees = (payload: any): Employee[] => {
  ...
  return rows.map((employee: any) => {
    ...
    return {
      employee_id:
        source.employee_id || source.employeeId || source.user_id || source.id,
...
export default function EmployeeProjectDetailsScreen() {
      ...
      try {
        ...
        if (tasksResponse.status === "fulfilled") {
          ...
          if (!Array.isArray(rows)) {
            ...
            else if (rows?.tasks && Array.isArray(rows.tasks))
              rows = rows.tasks;
            else rows = [];
          }
          ...
          let filteredRows = rows.filter((t: any) => {
            ...
            if (pId) {
              ...
              if (
                fetchedProject &&
                (pId === String(fetchedProject.id) ||
                  pId === String(fetchedProject.projectId) ||
                  pId === String(fetchedProject.project_id))
              ) {
                return true;
              }
            }
            ...
            if (
              !possibleName &&
              typeof t.project === "string" &&
              isNaN(Number(t.project))
            ) {
              possibleName = t.project;
            }
            const pName = possibleName.toLowerCase().trim();

            const currentProjName1 = String(fetchedProject?.title || "")
              .toLowerCase()
              .trim();
            const currentProjName2 = String(fetchedProject?.name || "")
              .toLowerCase()
              .trim();
            const currentProjName3 = String(fetchedProject?.project_name || "")
              .toLowerCase()
              .trim();

            if (
              pName &&
              (pName === currentProjName1 ||
                pName === currentProjName2 ||
                pName === currentProjName3)
            ) {
              return true;
            }
  ...
  const percent = Math.min(
    100,
    Math.max(0, Number(progress?.progress ?? project.overall_progress ?? 0)),
  );
  return (
      ...
      >
        ...
        <View className="mt-5 flex-row gap-3">
          <View className="flex-1 rounded-2xl border border-slate-200 bg-white p-4">
            ...
            <Text className="mt-2 text-xs text-slate-500">Budget</Text>
        ...
        <View className="flex-row flex-wrap gap-3">
          {[
            ...
            ["On Hold", progress?.onHold],
            ["Cancelled", progress?.cancelled],
          ].map(([label, value]) => (
            <View
              key={String(label)}
              className="w-[30%] rounded-xl border border-slate-200 bg-white p-3"
            >
              <Text className="text-xl font-black text-slate-900">
                {value ?? 0}
              </Text>
              <Text className="mt-1 text-[10px] font-bold uppercase text-slate-400">
                {label}
              </Text>
            </View>
          ))}
        ...
        <View className="gap-3 rounded-2xl border border-slate-200 bg-white p-4">
          <View className="flex-row gap-3">
            <View className="flex-1">
              <Info label="Category" value={project.project_category} />
            </View>
            <View className="flex-1">
          ...
          <View className="flex-row gap-3">
            ...
            <View className="flex-1">
              <Info label="Manager" value={project.project_manager} />
            </View>
          </View>
          ...
          {(() => {
            ...
            return allTasks.map((taskObj, index) => {
              const title =
                ...
                "Untitled Task";
              const tStatus =
                taskObj?.status ?? taskObj?.task_status ?? "Pending";
              ...
              return (
                <Pressable
                  key={String(taskId) + index}
                  onPress={() => router.push(`/employee/task/${taskId}`)}
                  className="rounded-2xl border border-slate-200 bg-white p-4"
                >
                  <View className="flex-row items-start justify-between">
                    <View className="flex-1 pr-3">
                      <Text
                        className={`font-bold text-slate-900 ${isDone ? "line-through opacity-50" : ""}`}
                      >
                        {title}
                      </Text>
                      {dueDate ? (
                    ...
                    <View
                      className={`rounded-lg px-2 py-1 ${isDone ? "bg-emerald-100" : "bg-blue-100"}`}
                    >
                      <Text
                        className={`text-[10px] font-bold uppercase ${isDone ? "text-emerald-700" : "text-blue-700"}`}
                      >
                        {tStatus}
                      </Text>
                    </View>
