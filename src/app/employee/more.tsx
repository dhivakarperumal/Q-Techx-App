      ...
      <ScrollView className="flex-1" contentContainerClassName="px-5 py-6">
        ...
        <View className="mt-6 gap-3">
          {options.map(([label, icon]) => (
            <Pressable key={label} className="flex-row items-center rounded-2xl border border-slate-200 bg-white p-4 active:bg-blue-50" onPress={() => Alert.alert(label, `${label} is ready to connect.`)}>
              <Ionicons name={icon} size={24} color="#2563eb" />
              <Text className="ml-3 flex-1 text-base font-semibold text-slate-900">{label}</Text>
