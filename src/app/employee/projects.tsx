...
const statuses = ["All", "Planning", "In Progress", "Testing", "On Hold", "Live", "Completed", "Cancelled"];
const statusColors: Record<string, { text: string; background: string; dot: string }> = {
...
const employeeReference = (user: Record<string, unknown> | null) => {
  if (!user) return "";
  return [user.employee_id, user.employeeId, user.user_id, user.id, user._id]
    .filter(Boolean)
    .map(String)[0] || "";
};
...
function StatusBadge({ status }: { status?: string }) {
  const style = statusColors[status || ""] || { text: "#64748b", background: "#f1f5f9", dot: "#94a3b8" };
  return <View className="flex-row items-center self-start rounded-full px-3 py-1.5" style={{ backgroundColor: style.background }}><View className="mr-2 h-2 w-2 rounded-full" style={{ backgroundColor: style.dot }} /><Text className="text-xs font-bold" style={{ color: style.text }}>{status || "Unknown"}</Text></View>;
}
...
export default function EmployeeProjectsScreen() {
  ...
  return <View className="flex-1 bg-slate-50">
    ...
    <ScrollView className="flex-1" contentContainerStyle={{ padding: 20, paddingBottom: 32 }} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => fetchProjects(true)} tintColor="#2563eb" />}>
      ...
      {loading ? <View className="items-center py-24"><ActivityIndicator size="large" color="#2563eb" /><Text className="mt-3 text-sm text-slate-500">Loading your projects...</Text></View> : error ? <View className="mt-6 rounded-2xl border border-rose-200 bg-rose-50 p-5"><Text className="font-semibold text-rose-700">{error}</Text><Pressable onPress={() => fetchProjects()} className="mt-4 self-start rounded-xl bg-rose-600 px-4 py-2"><Text className="font-bold text-white">Try again</Text></Pressable></View> : <View className="mt-5 gap-3">{filteredProjects.map((project) => <Pressable key={project.uuid} onPress={() => router.push(`/employee/projects/${project.uuid}` as any)} className="overflow-hidden rounded-[24px] border border-slate-200 bg-white p-0 shadow-sm active:bg-slate-50"><View className="border-b border-slate-100 bg-slate-50/80 px-4 py-4"><View className="flex-row items-start justify-between"><View className="mr-3 flex-1"><Text className="text-base font-black text-slate-900" numberOfLines={1}>{project.project_name || "Untitled project"}</Text><Text className="mt-1 text-xs font-semibold text-slate-400">{project.project_code || "Project"}{project.client_name ? `  ·  ${project.client_name}` : ""}</Text></View><StatusBadge status={project.current_status} /></View></View><View className="px-4 py-4"><View className="mb-3 flex-row items-center justify-between rounded-2xl bg-slate-50 px-3 py-2"><View className="flex-row items-center"><Ionicons name="calendar-outline" size={14} color="#64748b" /><Text className="ml-2 text-xs font-semibold text-slate-600">Started {formatDate(project.project_start_date)}</Text></View><View className="flex-row items-center"><Text className="mr-1 text-xs font-black text-blue-600">View details</Text><Ionicons name="chevron-forward" size={16} color="#2563eb" /></View></View><View className="flex-row items-center rounded-2xl border border-slate-100 bg-white px-3 py-3"><View className="mr-3 h-10 w-10 items-center justify-center rounded-2xl bg-orange-100"><Ionicons name="folder-open-outline" size={20} color="#ea580c" /></View><View className="flex-1"><Text className="text-sm font-bold text-slate-700">{project.client_name || "Client information pending"}</Text><Text className="mt-0.5 text-xs text-slate-400">{project.project_manager ? `Managed by ${project.project_manager}` : "Project progress is being tracked"}</Text></View></View></View></Pressable>)}{filteredProjects.length === 0 && <View className="rounded-2xl border border-dashed border-slate-300 bg-white p-6"><Text className="text-center font-semibold text-slate-600">No projects found</Text><Text className="mt-1 text-center text-sm text-slate-400">Try another search or status filter.</Text></View>}</View>}
    </ScrollView>
    <BottomHome />
  </View>;
}
