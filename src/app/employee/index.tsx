...
type Dashboard = {
  ...
  attendance?: { checkIn?: string | null; checkOut?: string | null; presentDays?: number; absentDays?: number; hoursThisWeek?: number };
  leaveBalance?: number;
  leaves?: { pendingCount?: number; recent?: { leave_type?: string; from_date?: string; no_of_days?: string | number; status?: string }[] };
  meetings?: { todayCount?: number; upcoming?: { id?: string | number; title?: string; startDate?: string }[] };
        ...
        <View className="rounded-3xl border border-orange-200 bg-orange-50 p-5">
          <View className="flex-row items-start justify-between">
            <View className="flex-1">
              <Text className="text-sm font-semibold text-orange-600">{new Date().toLocaleDateString(undefined, { weekday: "long", year: "numeric", month: "long", day: "numeric" })}</Text>
              <Text className="mt-2 text-3xl font-black text-slate-950">Good {new Date().getHours() < 12 ? "Morning" : new Date().getHours() < 17 ? "Afternoon" : "Evening"}, {displayName}!</Text>
              <Text className="mt-2 text-sm text-slate-600">Here&apos;s your work summary for today.</Text>
            </View>
            <Pressable onPress={() => fetchDashboard(true)} accessibilityLabel="Refresh dashboard" className="rounded-xl bg-white p-3"><Ionicons name="refresh-outline" size={20} color="#ea580c" /></Pressable>
          </View>
          <View className="mt-5 gap-3">
            <View className="flex-row items-center rounded-xl border border-white bg-white/70 px-3 py-3"><Ionicons name="checkmark-circle-outline" size={18} color="#16a34a" /><Text className="ml-2 text-sm font-semibold text-slate-700">Check-in: <Text className="text-green-600">{attendance.checkIn || "Not yet"}</Text><Text>  ·  </Text>Check-out: <Text className="text-amber-600">{attendance.checkOut || "Not yet"}</Text></Text></View>
            <View className="flex-row items-center rounded-xl border border-white bg-white/70 px-3 py-3"><Ionicons name="calendar-outline" size={18} color="#2563eb" /><Text className="ml-2 text-sm font-semibold text-slate-700">Leave Balance: <Text className="text-blue-600">{dashboard?.leaveBalance ?? 0} days left</Text></Text></View>
          </View>
        </View>
        <View className="mt-5 flex-row gap-3"><Metric label="Attendance This Month" value={attendance.presentDays ?? 0} icon="clipboard-outline" color="#16a34a" background="#f0fdf4" /><Metric label="Tasks Assigned" value={tasks.assigned ?? 0} icon="checkmark-square-outline" color="#2563eb" background="#eff6ff" /></View>
        <View className="mt-3 flex-row gap-3"><Metric label="Active Projects" value={projects.activeCount ?? 0} icon="folder-outline" color="#ea580c" background="#fff7ed" /><Metric label="Hours This Week" value={`${attendance.hoursThisWeek ?? 0}h`} icon="time-outline" color="#7c3aed" background="#f5f3ff" /></View>
        <View className="mt-3 flex-row gap-3"><Metric label="Pending Leaves" value={leaves.pendingCount ?? 0} icon="calendar-clear-outline" color="#ca8a04" background="#fefce8" /><Metric label="Next Pay Date" value={payroll.nextPayDate || "N/A"} icon="cash-outline" color="#059669" background="#ecfdf5" /></View>
        <View className="mt-3 flex-row gap-3"><Metric label="Meetings Today" value={dashboard?.meetings?.todayCount ?? 0} icon="videocam-outline" color="#db2777" background="#fdf2f8" /><Metric label="Overdue Tasks" value={tasks.overdue ?? 0} icon="alert-circle-outline" color="#dc2626" background="#fef2f2" /></View>
